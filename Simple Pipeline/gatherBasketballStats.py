...
def extractPlayerCareerStatsData(lname,fname):
    lname = lname.lower()
    fname = fname.lower()
    if len(lname) >= 5:
        lname = lname[0:5] 
    ...
    print(fullName)
    # still need to implement if the player is not found 
    ...
    print(player_df)
    player_df.to_excel(lname + fname + ".xlsx")
    return player_df

def getPlayerStatsSeason(lname, fname, season):
    player_df = extractPlayerCareerStatsData(lname, fname)
    playerSeasonStats = player_df[(player_df['Season'] == season)]
